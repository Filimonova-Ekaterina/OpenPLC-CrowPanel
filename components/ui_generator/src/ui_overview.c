...
static void create_hierarchy_card(ui_overview_t* overview, lv_obj_t* page, const data_model_equipment_t* root,
                                  bool flat_fallback);
...
static bool signal_already_selected(const ui_overview_t* overview, const data_model_tag_t* tag);
...
ui_overview_t* ui_overview_create(lv_obj_t* page, data_model_t* data_model)
{
    ...
    overview->equipment_binding_capacity = data_model_equipment_count(data_model);
...
static void create_hierarchy_cards(ui_overview_t* overview, lv_obj_t* page)
{
    size_t equipment_count = data_model_equipment_count(overview->data_model);
    if (equipment_count == 0) {
        ui_theme_create_heading(page, "Live equipment", "Built from OPC UA hierarchy");
    ...
    for (size_t index = 0; index < equipment_count; ++index) {
        ...
        if (equipment.parent_index == DATA_MODEL_INVALID_INDEX || equipment.parent_index >= equipment_count) {
            create_hierarchy_card(overview, page, &equipment, false);
            root_count++;
        }
    }
    if (root_count == 0) {
        ...
        if (data_model_get_equipment(overview->data_model, 0, &first)) {
            create_hierarchy_card(overview, page, &first, true);
        }
    }
}
                                  ...
                                  bool flat_fallback)
{
    ...
    for (size_t index = 0; index < equipment_count; ++index) {
        ...
        if (data_model_get_equipment(overview->data_model, index, &child) &&
            (flat_fallback || child.parent_index == root->index) && child.index != root->index) {
            child_count++;
        }
    }
    if (child_count == 0) {
    ...
    lv_label_set_text(title, "Live equipment");
    ...
    lv_label_set_text(detail, "Built from OPC UA hierarchy");
    ...
    for (size_t index = 0; index < equipment_count; ++index) {
        data_model_equipment_t child;
        if (! data_model_get_equipment(overview->data_model, index, &child) || child.index == root->index) {
            continue;
        }
        if (! flat_fallback && child.parent_index != root->index) {
            continue;
        }
        ...
        lv_coord_t x =
            columns > 1
                ? (lv_coord_t)(OVERVIEW_NODE_EDGE_PADDING + column *
                                                                (OVERVIEW_NODE_WRAPPER_WIDTH - OVERVIEW_NODE_RING_SIZE -
    ...
    if (! created_child) {
        create_equipment_node(overview, wrapper, root, (OVERVIEW_NODE_WRAPPER_WIDTH - OVERVIEW_NODE_RING_SIZE) / 2, 0);
    }
}
                                  ...
                                  lv_coord_t x, lv_coord_t y)
{
    ...
    lv_obj_set_style_border_color(binding->ring, lv_color_hex(UI_COLOR_INACTIVE), LV_PART_MAIN);
    lv_obj_set_style_border_width(binding->ring, 2, LV_PART_MAIN);
    lv_obj_set_style_radius(binding->ring, LV_RADIUS_CIRCLE, LV_PART_MAIN);
    ...
    lv_obj_set_size(binding->core, 19, 19);
    lv_obj_set_style_radius(binding->core, LV_RADIUS_CIRCLE, LV_PART_MAIN);
    ...
    lv_label_set_text(binding->state_label, "READY");
...
static bool signal_already_selected(const ui_overview_t* overview, const data_model_tag_t* tag)
{
    for (size_t index = 0; index < overview->signal_binding_count; ++index) {
        data_model_tag_t selected;
        if (data_model_get_tag(overview->data_model, overview->signal_bindings[index].tag_index, &selected) &&
            selected.semantic_role[0] != '\0' && strcmp(selected.semantic_role, tag->semantic_role) == 0) {
            return true;
        }
    }
    return false;
...
static void create_priority_signals(ui_overview_t* overview, lv_obj_t* page)
{
    static const char* PRIORITY_ROLES[] = {"pressure", "temperature",   "flow",   "demand",
                                           "power",    "motor_current", "runtime"};
    size_t tag_count                    = data_model_tag_count(overview->data_model);
    size_t selected_indices[OVERVIEW_PRIORITY_SIGNAL_COUNT];
    size_t selected_count = 0;

    for (size_t role_index = 0; role_index < sizeof(PRIORITY_ROLES) / sizeof(PRIORITY_ROLES[0]) &&
                                selected_count < OVERVIEW_PRIORITY_SIGNAL_COUNT;
         ++role_index) {
        for (size_t tag_index = 0; tag_index < tag_count; ++tag_index) {
            data_model_tag_t tag;
            if (data_model_get_tag(overview->data_model, tag_index, &tag) && tag_is_numeric_measurement(&tag) &&
                strcmp(tag.semantic_role, PRIORITY_ROLES[role_index]) == 0) {
                selected_indices[selected_count++] = tag.index;
                break;
            }
        }
    }
    for (size_t tag_index = 0; tag_index < tag_count && selected_count < OVERVIEW_PRIORITY_SIGNAL_COUNT; ++tag_index) {
        data_model_tag_t tag;
        if (! data_model_get_tag(overview->data_model, tag_index, &tag) || ! tag_is_numeric_measurement(&tag)) {
            continue;
        }
        overview->signal_binding_count = selected_count;
        for (size_t index = 0; index < selected_count; ++index) {
            overview->signal_bindings[index].tag_index = selected_indices[index];
        }
        if (! signal_already_selected(overview, &tag)) {
            selected_indices[selected_count++] = tag.index;
        }
    }
    overview->signal_binding_count = 0;
    ...
    lv_obj_align(title, LV_ALIGN_TOP_LEFT, 24, 0);
    lv_obj_t* detail = lv_label_create(card);
    lv_label_set_text(detail, "Priority signals selected automatically");
    lv_obj_set_style_text_color(detail, lv_color_hex(UI_COLOR_TEXT_SECONDARY), LV_PART_MAIN);
    ...
    for (size_t selected = 0; selected < selected_count; ++selected) {
        ...
        lv_obj_align(name, LV_ALIGN_TOP_LEFT, 66, 8);
        ...
        lv_obj_align(binding->value_label, LV_ALIGN_BOTTOM_LEFT, 66, -7);
    }
}
...
void ui_overview_update(ui_overview_t* overview)
{
    ...
    for (size_t index = 0; index < overview->equipment_binding_count; ++index) {
        ...
        if (! binding->displayed_valid || binding->displayed_running != running || binding->displayed_alarm != alarm) {
            ...
            lv_obj_set_style_border_opa(binding->ring, alarm || running ? LV_OPA_COVER : LV_OPA_70, LV_PART_MAIN);
            ...
            ui_theme_set_label_text(binding->state_label, alarm ? "ALARM" : (running ? "RUNNING" : "READY"));
    ...
    for (size_t equipment_index = 0; equipment_index < equipment_count; ++equipment_index) {
        ...
        if (ui_theme_find_tag_by_role(overview->data_model, equipment_index, "operating_status", &status) &&
    ...
    } else if (data_model_equipment_count(overview->data_model) == 0) {
