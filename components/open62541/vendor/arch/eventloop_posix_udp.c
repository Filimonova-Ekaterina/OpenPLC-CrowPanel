...
/* ESP-IDF does not expose the POSIX getifaddrs API. The HMI uses OPC UA TCP;
 * keep UDP available for unicast operation and reject only explicit multicast
 * interface selection instead of adding a platform-specific address shim. */
static UA_StatusCode
setMulticastInterface(const char *netif, struct addrinfo *info,
                      MulticastRequest *req, const UA_Logger *logger) {
    (void)netif;
    ...
    UA_LOG_WARNING(logger, UA_LOGCATEGORY_NETWORK,
                   "UDP multicast interface selection is unavailable on ESP-IDF");
...
#endif /* UA_ARCHITECTURE_WIN32 / ESP_PLATFORM */
