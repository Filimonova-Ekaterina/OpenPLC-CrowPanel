...
esp_err_t bsp_audio_init(const i2s_std_config_t *i2s_config)
{
...

    {
        ...
        err = i2s_new_channel(&rx_chan_cfg, NULL, &i2s_rx_chan);
        if (err != ESP_OK)
            return err;
    ...
    }
    {
        ...
        err = i2s_new_channel(&chan_cfg, &i2s_tx_chan, NULL);
        if (err != ESP_OK)
            return err;
...
esp_codec_dev_handle_t bsp_audio_codec_speaker_init(void)
{
    ...
    // esp_codec_dev_set_vol_handler(dev, vol_if);

    // esp_codec_dev_vol_map_t volume_maps[] = {
    //     {.vol = 1, .db_value = -49.5},
    ...
    // esp_codec_dev_set_vol_curve(dev, &vol_curve);
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config, bsp_lcd_handles_t *ret_handles)
{
    ...
    esp_lcd_dpi_panel_config_t dpi_config = {
        ...
        .virtual_channel = 0,
        .in_color_format = ESP_COLOR_FOURCC_RGB16,
        .out_color_format = ESP_COLOR_FOURCC_RGB16,
        .num_fbs = 1,
        .video_timing = {
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    esp_err_t err = esp_lcd_touch_new_i2c_gt911(tp_io_handle, &tp_cfg, ret_touch);
    return ESP_OK;
}
...
static lv_indev_t *bsp_display_indev_init(lv_display_t *disp)
{
    ...
    if (indev && indev->driver) {
        indev->driver->scroll_limit = 6;
        indev->driver->long_press_time = 300;
        indev->driver->long_press_repeat_time = 80;
        indev->driver->gesture_limit = 40;
