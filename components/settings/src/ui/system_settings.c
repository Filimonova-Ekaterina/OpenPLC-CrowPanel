...
static lv_obj_t* add_slider(lv_obj_t* card, const char* symbol, int minimum, int maximum, int value,
                            lv_event_cb_t callback)
{
    ...
    lv_obj_set_size(slider, lv_pct(55), 18);
    lv_obj_align_to(slider, value_label, LV_ALIGN_OUT_LEFT_MID, -20, 0);
    ...
    lv_obj_set_style_bg_color(icon_badge, lv_color_hex(0x181818), LV_PART_MAIN);
    lv_obj_set_style_bg_opa(icon_badge, LV_OPA_COVER, LV_PART_MAIN);
    lv_obj_set_style_border_color(icon_badge, lv_color_hex(0x555555), LV_PART_MAIN);
    lv_obj_set_style_border_width(icon_badge, 1, LV_PART_MAIN);
    lv_obj_set_style_radius(icon_badge, LV_RADIUS_CIRCLE, LV_PART_MAIN);
    lv_obj_set_style_pad_all(icon_badge, 0, LV_PART_MAIN);
    lv_obj_align_to(icon_badge, slider, LV_ALIGN_OUT_LEFT_MID, -14, 0);

    lv_obj_t* icon = lv_label_create(icon_badge);
    lv_label_set_text(icon, symbol);
    lv_obj_set_style_text_color(icon, lv_color_hex(0x6BC1FF), LV_PART_MAIN);
...
{
    lv_obj_t* card = create_card(parent, "Sleep after", 132);
    lv_obj_t* button_row = lv_obj_create(card);
    lv_obj_remove_style_all(button_row);
    lv_obj_set_size(button_row, lv_pct(64), 88);
...
lv_obj_t* system_settings_create(lv_obj_t* parent)
{
    ...
    lv_obj_t* brightness_card = create_card(root, "Brightness", 90);
    add_slider(brightness_card, LV_SYMBOL_EYE_OPEN, 1, 100,
               settings_config_load_brightness(), brightness_event);
    lv_obj_t* volume_card = create_card(root, "Volume", 90);
    add_slider(volume_card, LV_SYMBOL_VOLUME_MAX, 0, 100,
               settings_config_load_audio_volume(), volume_event);
